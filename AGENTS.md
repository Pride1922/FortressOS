...
History lives in [ROADMAP.md](ROADMAP.md); qualifications and technical debt in [ARCH_REVIEW.md](ARCH_REVIEW.md). Keep new implementation instructions here, checkpoint history there, and verification claims tied to actual evidence.

| Checkpoint | Status / acceptance |
| --- | --- |
| Latest recorded completion: Phase 9G.5b SuperSpeed enumeration & BOT transport | SuperSpeed (USB 3.x) mass storage now works end-to-end on physical hardware. USB 3.0 ports are reset and PLS-polled; the slot context carries speed 4; EP0 packet size is 512; descriptor parsing handles the endpoint companion descriptor and the `bMaxBurst` field; two descriptor-walk bugs (loop hang on non-endpoint descriptors, UAS alternate overwriting BOT endpoints) were found and fixed. Verified on Dell Latitude 5590 with SanDisk USB 3.2 Gen 1 (0781:5588) on Port 0x12: SuperSpeed enumeration, SCSI INQUIRY ("SanDisk" / "3.2 Gen 1"), READ CAPACITY (117,864 MiB), GPT parse, ext2 mounted read-write at `/mnt`, classification `SYNC_BACKED`. Also verified: 5590 with one xHCI controller, 5530 with two, USB 2.0 devices on both. See ROADMAP.md §9G.5b. |
...
| Phase 9G.4 USB writable persistence & durability classification | COMPLETE (2026-09-19): BOT stall recovery with bounded endpoint reset, MODE SENSE(6/10) cache-policy discovery, four-tier durability classification (SYNC_BACKED / WRITE_THROUGH / ASSUMED_WRITE_THROUGH / READ_ONLY), explicit usb_data_mode=rw opt-in, and a normal `sync` path distinct from ext2_sync_all(). `/mnt` mounts read-write on physical USB; files persist across a full power cycle; e2fsck -fn clean. Kingston 13FE:4200 classifies as ASSUMED_WRITE_THROUGH (no caching page reported). Strong paths (WRITE_THROUGH, SYNC_BACKED) are QEMU-verified only pending SuperSpeed. |
| Phase 9G.5b SuperSpeed enumeration & BOT transport | COMPLETE (2026-09-20): USB 3.0 ports reset and PLS-polled; SuperSpeed slot context, EP0 packet size 512, descriptor parsing, endpoint companion handling, and bulk endpoint configuration all working. Two descriptor-walk bugs fixed: `off += len` was inside the endpoint branch (loop hang), and `found_bot_if` was not cleared on non-BOT interfaces (UAS alternate endpoints overwrote BOT endpoints, causing USB Transaction Error). Verified on Dell Latitude 5590 with SanDisk USB 3.2 Gen 1 (0781:5588) on Port 0x12: SuperSpeed enumeration, SCSI INQUIRY, READ CAPACITY (241,385,472 sectors, 117,864 MiB), GPT parse, ext2 mounted RW at /mnt, classification SYNC_BACKED. See ROADMAP.md §9G.5b. |
| Phase 9H RAM capacity | COMPLETE (2026-09-20): PMM bitmap extended from 64 KiB to 1 MiB, covering 32 GiB of physical RAM (was 2 GiB). Limine's HHDM measured to cover only ~2.5 GiB on the Dell 5590; `vmm_init` maps the full 32 GiB HHDM itself, but page-table construction runs on Limine's CR3 and reaches new tables through `phys_to_virt`, which uses the currently-loaded HHDM. Two-stage PMM init added: allocation capped at 1 GiB until `vmm_init` completes the CR3 switch, then `pmm_unlock_high_memory()` lifts the cap. Verified on Dell 5590 (32 GiB): `dmesg` reports `Total Physical RAM: 32768 MiB (8388608 frames)`, `Usable Free RAM: 31873 MiB`; write-readback probe passed at 2, 4, 16, and 30 GiB; boot log preserved at `/mnt/boot.log`. See ROADMAP.md §9H. |
| Next: milestone of your choice | 9G.5c/d (strong durability on a second device class, persistence on the SanDisk) are unblocked by 9G.5b. Other candidates: introspection syscalls + `sysinfo`/`top`/`ps`, persistent rootfs with `/paradise`, shell improvements, MicroPython. |

### Phase 9G implementation handoff

Phase 9G is complete through 9G.5b. The staged plan, per-stage acceptance evidence, and hardware observations are recorded in [ROADMAP.md](ROADMAP.md). The driver handles USB 2.0 and USB 3.x direct-attached mass storage on any enumerated xHCI controller; USB 3.x devices enumerate as SuperSpeed and complete BOT transport. Hubs, hot-plug, UAS, and non-mass-storage classes remain out of scope. See the "What 9G does NOT do" list below.
...
- Preserve historical results and label new evidence by command, firmware, image/device and result. QEMU USB success does not establish Dell USB acceptance. Update this status and ROADMAP.md after each completed stage.
...
These are manual hardware observations, supplementing the automated QEMU and host test suites. Cached text scrolling avoids framebuffer reads; keep early/no-UART output working. Detailed console, NMI and input test notes are in [ROADMAP.md](ROADMAP.md).
