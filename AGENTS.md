...
| --- | --- |
| Latest recorded completion: Phase 9G.5a multiple xHCI controllers | FortressOS enumerates and initializes every xHCI controller the platform exposes, and reaches a mass-storage device on whichever controller it's attached to. Verified on Dell 5590 (one controller, unchanged) and Dell 5530 (two controllers, stick found and mounted read-write on controller 2). Commits: xhci_controller_t array, pci_find_all_devices() bounded enumeration, per-controller init loop with s_active_usb_controller selection. See ROADMAP.md §9G.5a. |
...
| Next: Phase 9G.5b SuperSpeed enumeration | USB 3.x port link state and SuperSpeed slot/endpoint context layout. Required to reach the SanDisk stick whose cache policy is currently unverified. Test target: SanDisk enumerates on Port 0x12 as USB 3.0 BOT with speed=SuperSpeed, then its MODE SENSE / SYNCHRONIZE CACHE responses can be read. |

### Phase 9G implementation handoff

Phase 9G is complete. The staged plan, per-stage acceptance evidence,
and hardware observations are recorded in [ROADMAP.md](ROADMAP.md). The
active checkpoint is Phase 9G.5b (SuperSpeed enumeration); the driver
handles USB 2.0 direct-attached mass storage on any enumerated xHCI
controller and skips SuperSpeed devices cleanly.

#### What 9G does NOT do

- No USB 3.x SuperSpeed support; direct-attached SuperSpeed is follow-up work after 9G acceptance. Do not assume automatic USB 2.0 fallback.
...
| H10 | SanDisk USB 3.x stick. Enumerates as a SuperSpeed device on Port 0x12 (Dell 5590) and Port 0x10 (Dell 5530). Skipped by current scope on both machines. SuperSpeed support (9G.5b) is required to reach the BOT layer with this device. |
