...
| Current: multi-core (SMP) support | Pieces 1–4 COMPLETE (2026-09-24); verified on QEMU (BIOS & UEFI, 1/4/8 CPUs) and Dell Latitude 5590 hardware ([Piece 4 details](docs/roadmap/smp-piece4-scheduler.md), [Piece 3 details](docs/roadmap/smp-piece3-lock-discipline.md)). APs actively schedule, unpark, run 100 Hz LAPIC timers, execute pinned workers, and perform dual-lock work-stealing across cores (13 tasks stolen across APs 1..7 on Dell 5590). Full design, sequencing, and binding invariants (SM IDs) in [`SMP_DESIGN.md`](SMP_DESIGN.md). Piece 5 (Cross-Core Coordination & IPIs) is next. Smaller open items not blocking SMP: 9G.5c/d (strong durability on a second device class, persistence on the SanDisk), introspection syscalls + `sysinfo`/`top`/`ps`, persistent rootfs with `/paradise`, shell improvements, MicroPython. |
